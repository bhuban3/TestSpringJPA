...
public class TestServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String numStr = request.getParameter("numberOfStudents");
        String action = request.getParameter("action");

        int numberOfStudents = 0;
        if (numStr != null && !numStr.isEmpty()) {
            numberOfStudents = Integer.parseInt(numStr);
        }
        ...
        switch (action) {
            ...
            case "Insert":
                if (service.insertDB(deptName, numberOfStudents)) {
            ...
            case "View":
                List<String> list = service.viewDB();
                if (list.isEmpty()) {
                    ...
                    for (String row : list) {
                        // Format: Department: CSE, Number of Students: 120
                        String[] parts = row.split(", Number of Students: ");
                        String dept = parts[0].replace("Department: ", "");
                        String num = parts.length > 1 ? parts[1] : "";

                        out.println("<tr><td>" + dept + "</td><td>" + num + "</td></tr>");
                    }
                    out.println("</table>");
                }
                break;
            ...
            case "Update":
                if (service.updateDB(deptName, numberOfStudents)) {
